...
struct CtomlCr::Any
  ...
  def dig?(index_or_key : String | Int, *subkeys) : CtomlCr::Any?
    self[index_or_key]?.try &.dig?(*subkeys)
  end

  # :nodoc:
  def dig?(index_or_key : String | Int) : CtomlCr::Any?
    case @raw
    when Hash, Array
      self[index_or_key]?
    else
  ...
  # Returns a new CtomlCr::Any instance with the `raw` value `dup`ed.
  def dup
    Any.new(raw.dup)
  end

  # Returns a new CtomlCr::Any instance with the `raw` value `clone`ed.
  def clone
    Any.new(raw.clone)
  end
end
